import createGeomBuilder from "../index.js";
...
const clearColor = [0.9, 0.9, 0.9, 1];
...
function createGeometryFromArrays() {
  ...
  for (let x = 0; x < N; x++) {
    for (let z = 0; z < N; z++) {
      ...
      const color = HsluvToRgb([200 + 80 * a[1] * 4, 60, 60 + 30 * a[1] * 4]);
...
function createGeometryFromTypedArrays() {
  ...
  for (let x = 0; x < N; x++) {
    for (let z = 0; z < N; z++) {
      ...
      const color = HsluvToRgb([300 + 80 * a[1] * 4, 60, 60 + 30 * a[1] * 4]);
...
function createGeometryFromTypedArraysNoGC() {
  ...
  for (let x = 0; x < N; x++) {
    for (let z = 0; z < N; z++) {
      ...
      const color = HsluvToRgb([100 + 80 * (1 - ay * 4), 60, 60 + 30 * ay * 4]);
      positions[i * 3 + 0] = ax;
      positions[i * 3 + 1] = ay;
      positions[i * 3 + 2] = az;
      colors[i * 4 + 0] = color[0];
...
const g6bg = createGeometryFromGeomBuilderWithCells(null, clearColor, -0.01);
const landscape6bg = {
  attributes: {
...
const g6 = createGeometryFromGeomBuilderWithCellsAsLines();
const landscape6 = {
  attributes: {
...
const g6p = createGeometryFromGeomBuilderWithCellsAsPoints();
const landscape6p = {
  attributes: {
...
const g7 = createGeometryFromGeomBuilderWithCellsAsColumns();
const landscape7 = {
  attributes: {
...
ctx.frame(() => {
  ...
  if (frame === 3) {
    console.log(
      "geometry from geom builder render time",
      query4.result / 1000000,
    );
  }
  ...
  if (frame === 3) {
    console.log(
      "geometry from geom builder with indices render time",
      query5.result / 1000000,
    );
  }
